...
################## Lectura #############
library(data.table)
BAM_Normal_procesado <- fread("Secuencias_SRR1751230_STAR.txt",sep = "\t",select = c(1:6))
...
cromosomas <- unique(BAM_Normal_procesado_inserciones$rname)
...
BAM_Normal_procesado_inserciones <- BAM_Normal_procesado[insercciones,]
BAM_Normal_procesado_inserciones <- na.omit(BAM_Normal_procesado_inserciones)
...
buscar_INDELS <- function(valor,longitud,tipo,bam,bam2,media_long,genes_hg19){
  ...
  probosita$final <- longitud2_cumsum[test1] 
  bases <- with(probosita,substr(seq,(inicio),(final)))
  rango_bases <- with(probosita,substr(seq,(inicio-5),(final+5)))
  
  matriz_inserciones5 <- data.frame(cbind(longitud_inserccion,chr,pos_inicio,pos_final,bases,rango_bases))
  matriz_inserciones5 <- na.omit(matriz_inserciones5)
  library(plyr)
  matriz_inserciones6 <- ddply(matriz_inserciones5,.(longitud_inserccion,chr,pos_inicio,pos_final,bases,rango_bases),nrow)
  #matriz_inserciones6 <- matriz_inserciones6[-which(matriz_inserciones6$longitud_inserccion==0),]
  matriz_inserciones6$pos_final <- as.numeric(matriz_inserciones6$pos_final)
  matriz_inserciones6$pos_inicio <- as.numeric(matriz_inserciones6$pos_inicio)
  colnames(matriz_inserciones6)[7] <- "Numero_Mutadas"
  matriz_inserciones6 <- matriz_inserciones6[which(matriz_inserciones6$Numero_Mutadas>=5),]
  matriz_inserciones6 <- as.data.table(matriz_inserciones6)
  matriz_inserciones6$pos_final <- matriz_inserciones6$pos_final
  
  totales <- list()
  chr <- matriz_inserciones6$chr
  pos_inicio <- matriz_inserciones6$pos_inicio
  pos_final <- matriz_inserciones6$pos_final
  for (n in 1:nrow(matriz_inserciones6)){
    print(n)
    totales[[n]] <- nrow(BAM_Normal_procesado[rname==chr[n] & end>=pos_final[n] & pos<= pos_inicio[n]])
  }
  totales <- unlist(totales)
  matriz_inserciones6$totales <- totales
  return(matriz_inserciones6)
  }
